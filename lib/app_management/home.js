let libraryLocation = '../../lib/config/app.json'
...
function loadLevelPane(level) {
    ...
    $('#load_game_level').bind('click', () => {
        window.location.href = `http://127.0.0.1:5500/game/index.html?${level}`
    })
...
function getEntries(type) {
    $.get(`../../lib/config/user/${type}.json`, json => {
        ...
        let table_id
        type == 'leaderboard' ? table_id = 'leaderboard-entries' : table_id = 'history-entries'
