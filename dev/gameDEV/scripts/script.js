...
function setup() {
    ...
    $.get('../../pkg/JSON/testLevel.json', function(data) {
        for(let i = 0; i <= 4; i++) {
            let spawn_point = data.spawnable_points[floor(random(data.spawnable_points.length))]
            
            planes.push(new Plane(spawn_point, 'A320'))
            planes[i].makeCallsign()
            data.spawnable_points.splice(data.spawnable_points.indexOf(spawn_point), 1)
        }
    })
...
function draw() {
    ...
    for(let i = 0; i < planes.length; i++) {
        ...
        if(planes[i].path_to_destination.length != 0 && permit_path == true) {
            ...
            if([grid_x, grid_y] != path && grid.is_a_neighbour(grid_x, grid_y, pathing_plane)) {
