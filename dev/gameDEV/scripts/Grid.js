class Grid {
    ...
    show_areas(type) {
        ...
        switch (type) {
            case 'move':
                fill('red')
                area = this.moveable_areas
                break
            case 'spawn':
                fill('green')
                area = this.spawn_areas
                break
            default:
                area = this.moveable_areas

        }
        for(let vector = 0; vector < area.length; vector++) {
            // console.log(vector[0])
            rect(area[vector][0] * this.grid_size, area[vector][1] * this.grid_size, this.grid_size, this.grid_size)
        }
    }
}