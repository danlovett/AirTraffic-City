        ...
        clientDB.all(`SELECT * FROM history LEFT JOIN users ON history.personID = users.id WHERE users.id = ${req.query.id} ORDER BY history.date DESC`, [], (err, joins) => { 
            if(user != null) {      
                clientDB.all(`SELECT * FROM leaderboard WHERE personID = ${user["id"]}`, [], (err, leaderboard) => {
                    ...
                    res.render('profile', { load_user: user, current_user: req.user, history: joins, last: joins.sort( (a,b) => a.date.split(' ')[1] - b.date.split(' ')[1] )[0], best: joins.sort( (a,b) => b.score - a.score )[0], leaderboard_entry: leaderboard, id_types: ['Level', 'Date', 'Score'], error: req.query.error, message: req.query.message })
                })
            } else {
